...
if (@$_GET['q'] == 6)
    echo 'class="active"';
?>><a href="dash.php?q=6">Help</a></li>
<li <?php
if (@$_GET['q'] == 7)
    echo 'class="active"';
?>><a href="dash.php?q=7">Help view</a></li>

<li <?php
if (@$_GET['q'] == 8)
    echo 'class="active"';
?>><a href="dash.php?q=8">result view</a></li>
      </ul>
          </div>
  </div>
</nav>
...
<tr><td style="vertical-align:middle"><b>Rank</b></td><td style="vertical-align:middle"><b>Name</b></td><td style="vertical-align:middle"><b>Email</b></td><td style="vertical-align:middle"><b>Username</b></td><td style="vertical-align:middle"><b>Mobile Number</b></td><td style="vertical-align:middle"><b>Address</b></td><td style="vertical-align:middle"><b>Score</b></td></tr>';
    $c = $showfrom-1;
    ...
    if($total >= $showfrom){
        ...
        while ($row = mysqli_fetch_array($q)) {
          ...
          echo '<tr><td style="color:#99cc32"><b>' . $c . '</b></td><td style="vertical-align:middle">' . $name . '</td><td style="vertical-align:middle">' . $email . '</td><td style="vertical-align:middle">' . $username . '</td><td style="vertical-align:middle">' . $phno . '</td><td style="vertical-align:middle">' . $address . '</td><td style="vertical-align:middle">' . $s . '</td><td style="vertical-align:middle">';
        }
    }
    else{
    }
...
<tr><td style="vertical-align:middle"><b>S.N.</b></td><td style="vertical-align:middle"><b>Name</b></td><td style="vertical-align:middle"><b>Email</b></td><td style="vertical-align:middle"><b>Username</b></td><td style="vertical-align:middle"><b>Mobile Number</b></td><td style="vertical-align:middle"><b>Address</b></td><td style="vertical-align:middle"></td></tr>';
    $c = 1;
    while ($row = mysqli_fetch_array($result)) {
                $name     = $row['name'];
                $email    = $row['email'];
        ...
        echo '<tr><td style="vertical-align:middle">' . $c++ . '</td><td style="vertical-align:middle">' . $name . '</td><td style="vertical-align:middle">' . $email . '</td><td style="vertical-align:middle">' . $username . '</td><td style="vertical-align:middle">' . $phno . '</td><td style="vertical-align:middle">' . $address . '</td>
  <td style="vertical-align:middle"><a title="Delete User" href="update.php?dusername=' . $username . '"><b><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></b></a></td></tr>';
    }
    $c = 0;
    echo '</table></div>';
...
if (isset($_POST['help'])) {
    $name=$_POST['name'];
    $ans=$_POST['ans'];
    $insrt="INSERT INTO `question` (`title`,`ans`) values('$name','$ans' )";
    $query=mysqli_query($con,$insrt);
    if ($query) {
...
<div class="col-md-6">
    <form action="" method="POST">
        ...
        <textarea name="ans" class="form-control" style="height:200px;"></textarea><br>
        <button type="submit" class="form-control btn btn-danger" name="help">submit</button>
    </form>
 </div>
<div class="col-md-3"></div>
...
if (@$_GET['q'] == 7 && !(@$_GET['step'])) {
     $qrs=mysqli_query($con,"SELECT * FROM `question`");
  $html = '<table class="table" id="viewdata">
    <thead>
        <tr>
        <th scope="col">id</th>
          <th scope="col">title</th>
          <th scope="col">Answar</th>
...
while($row = mysqli_fetch_assoc($qrs)) {
    $html .='<tr>
      ...
      <td>
          <button id="edit" class="btn btn-sm btn-info">Edit</button>
          <a data-id="'.$row["id"].'" onclick="conDelete(this)" class="btn btn-sm btn-danger">Delete</a>
      </td>
...
if (isset($_POST['status_dc'])) {
   $update="UPDATE `status` SET `status`=0";
    $query=mysqli_query($con,$update);
    
}
if (isset($_POST['status_ac'])) {
   $update="UPDATE `status` SET `status`=1";
    $query=mysqli_query($con,$update);
...
if (@$_GET['q'] == 8 && !(@$_GET['step'])) {
    $qrs=mysqli_query($con,"SELECT * FROM `status`");
    $fatch=mysqli_fetch_assoc($qrs);
    if($fatch["status"]==1){
        echo '<div class="row">
    <div class="col-md-3"></div>
<div class="col-md-6">
    ...
    <input type="submit" class="btn btn-danger" name="status_dc" value="deactive" class="form-control">
...
<div class="col-md-6">
    ...
    <input type="submit" class="btn btn-danger" name="status_ac" value="active" class="form-control">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
            ...
            <input type="text" id="title" name="title" class="form-control" placeholder="this is ">
            <br>
           <textarea cols="3" id="describe" name="describe" rows="3" class="form-control"></textarea>
            <br>

        
      </div>
      <div class="modal-footer">
       <!--  <button type="button" id="saveBtn" class="btn btn-primary saveBtn"  onclick="store()">Save changes</button>
 -->        <button type="button" class="btn btn-primary updateBtn" onclick="update()">Update changes</button> 
...
$('table').on('click','#edit',function(){
    modal.find('.modal-title').text('this is updateBtn');
    /*updateBtn.show();
    saveBtn.hide();*/
    modal.modal('show');
    ...
    $('textarea[name="describe"]').val(email);
